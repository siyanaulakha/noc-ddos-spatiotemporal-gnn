#!/usr/bin/env python3
"""
V5 P2-G1B-R3 Source-Only Graph Baseline Implementation Smoke Test

This append-only revision preserves the historical G1B, G1B-R1, and G1B-R2
HOLD outputs. It fixes the remaining R2 prerequisite-path bug: R2 accidentally
searched the G1B-R1 directory for an R2-named lock file.
...
STAGE = "V5_P2_G1B_R3_SOURCE_ONLY_GRAPH_IMPLEMENTATION_SMOKE"
...
def main() -> int:
    ...
    report = {
        ...
        "next_stage": (
            "V5_P2_G1C_SOURCE_ONLY_GRAPH_TRAINING_MATRIX"
            if not failures
            else "V5_P2_G1B_R4_IMPLEMENTATION_FIX"
        ),
    }
    ...
    contract_path = (
        output_dir
        / "V5_P2_G1B_R3_SOURCE_ONLY_GRAPH_IMPLEMENTATION_CONTRACT.json"
    )
    if resolved_contract:
        write_json(contract_path, resolved_contract)

    markdown = [
        "# V5 P2-G1B-R3 Source-Only Graph Implementation Smoke",
        "",
    ...
    atomic_write(
        output_dir
        / "V5_P2_G1B_R3_SOURCE_ONLY_GRAPH_IMPLEMENTATION_SMOKE.md",
        "\n".join(markdown),
    )
    ...
    write_json(
        output_dir
        / "V5_P2_G1B_R3_SOURCE_ONLY_GRAPH_IMPLEMENTATION_SMOKE_LOCK.json",
        lock,
    )
    ...
    if failures:
        ...
        print("===== V5 P2-G1B-R3 SOURCE-ONLY GRAPH SMOKE =====")
        print("status: HOLD")
        print("failure_count:", len(failures))
        print("warning_count:", len(warnings))
        for failure in failures:
            print("FAIL:", failure)
        print("next_stage: V5_P2_G1B_R4_IMPLEMENTATION_FIX")
        print(f"{STAGE}_HOLD")
        return 1
