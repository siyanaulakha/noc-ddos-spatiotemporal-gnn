...
SCRIPT="$REPO/scripts/v5/p2/smoke_v5_p2_g1b_r1_source_only_graph_implementations.py"
OUT="$REPO/reports/v5/p2_g1b_r1_source_only_graph_implementation_smoke"
LOG="$REPO/logs/v5/v5_p2_g1b_r1_source_only_graph_implementation_smoke.log"

echo "===== V5 P2-G1B-R1 SOURCE-ONLY GRAPH SMOKE ====="
echo "repo=$REPO"
echo "g0=$G0"
echo "r1=$R1"
...
echo "smoke_optimizer_steps_per_operator=1"
...
for path in "$G0" "$R1" "$TOPOLOGY" "$G1B_HOLD" "$DATA"; do
    [ -d "$path" ] || {
...
echo
echo "v5_p2_g1b_r1_status=$status"

if [ -f "$OUT/V5_P2_G1B_R1_SOURCE_ONLY_GRAPH_IMPLEMENTATION_SMOKE_COMPLETE" ]; then
    cat "$OUT/V5_P2_G1B_R1_SOURCE_ONLY_GRAPH_IMPLEMENTATION_SMOKE_COMPLETE"
elif [ -f "$OUT/V5_P2_G1B_R1_SOURCE_ONLY_GRAPH_IMPLEMENTATION_SMOKE_HOLD" ]; then
    cat "$OUT/V5_P2_G1B_R1_SOURCE_ONLY_GRAPH_IMPLEMENTATION_SMOKE_HOLD"
else
