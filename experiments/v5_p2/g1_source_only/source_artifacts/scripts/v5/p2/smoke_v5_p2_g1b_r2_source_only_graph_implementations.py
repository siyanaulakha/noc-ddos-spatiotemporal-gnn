#!/usr/bin/env python3
"""
V5 P2-G1B-R2 Source-Only Graph Baseline Implementation Smoke Test

This append-only revision preserves both the historical G1B HOLD and G1B-R1
HOLD. It fixes the R1 prerequisite-path bug: R1 accidentally searched the
original G1B directory for an R1-named lock file.
...
STAGE = "V5_P2_G1B_R2_SOURCE_ONLY_GRAPH_IMPLEMENTATION_SMOKE"
...
def main() -> int:
    ...
    report = {
        ...
        "next_stage": (
            "V5_P2_G1C_SOURCE_ONLY_GRAPH_TRAINING_MATRIX"
            if not failures
            else "V5_P2_G1B_R3_IMPLEMENTATION_FIX"
        ),
    }
    ...
    contract_path = (
        output_dir
        / "V5_P2_G1B_R2_SOURCE_ONLY_GRAPH_IMPLEMENTATION_CONTRACT.json"
    )
    ...
    atomic_write(
        output_dir
        / "V5_P2_G1B_R2_SOURCE_ONLY_GRAPH_IMPLEMENTATION_SMOKE.md",
        "\n".join(markdown),
    )
    ...
    if failures:
        ...
        print("===== V5 P2-G1B-R2 SOURCE-ONLY GRAPH SMOKE =====")
        print("status: HOLD")
        print("failure_count:", len(failures))
        print("warning_count:", len(warnings))
        for failure in failures:
            print("FAIL:", failure)
        print("next_stage: V5_P2_G1B_R3_IMPLEMENTATION_FIX")
        print(f"{STAGE}_HOLD")
        return 1
